...
<table class="layui-table" lay-data="{width: 920,url:'<%=basePath%>attence/getMettingSignInfo', page:true, id:'idTest'}" lay-filter="demo">
  <thead>
    <tr >
       <!-- <th lay-data="{type:'checkbox'}"></th> -->
      <th lay-data="{field:'mettingId', width:110, sort: true}">会议编号</th>
<!--       <th lay-data="{field:'photo', width:210}">照片</th> -->
      <th lay-data="{field:'mettingName', width:110}">会议名称</th>
      <th lay-data="{field:'employeeId', width:100}">工号</th>
      <th lay-data="{field:'empName', width:100}">姓名</th>
      <th lay-data="{field:'signDate', width:180}">签到时间</th>
      <th lay-data="{field:'photo', width:100}">签到照片</th>
  ...
  table.on('tool(demo)', function(obj){
    ...
    if(obj.event === 'detail'){
      ...
      layer.confirm('确认删除么', function(index){
        ...
        $.ajax({
        	type:"POST",
			url:"<%=basePath%>attence/deleteMetSignInfo",
			data:{"id":data.id},
			dataType:"json",
		    success:function(data){
		    	...
		    	layer.msg('删除成功' , {time:1000}, function(){
		    		window.location.href="<%=basePath%>attence/mettingSign";
		    		});
		    	}
		    },
		    error:function(msg){
