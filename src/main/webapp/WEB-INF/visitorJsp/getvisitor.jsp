...
<table class="layui-table" lay-data="{width:1146 ,  url:'<%=basePath%>vistor/getvisitors', page:true, id:'idTest' }" lay-filter="demo">
  <thead>
    <tr>
...
<script type="text/html" id="barDemo">
  <a class="layui-btn layui-btn-danger layui-btn-xs" lay-event="detail">加入黑名单</a>
  </script>
<script>
...
//监听工具条
  table.on('tool(demo)', function(obj){
    ...
    if(obj.event === 'detail'){
     ...
     $.ajax({
    	 type:'POST',
    	 url:'<%=basePath%>vistor/setblackList?visitorId='+data.id,
    	 dataType:"text",
 		 success:function(data){
 		   layer.msg('提交成功' , {time:1000}, function(){
 		    		window.location.href="<%=basePath%>vistor/getvisitor";
 		    		});
    ...
    reload: function(){
     ...
     if(visitInfoSereach == 'visitorId'){
      ...
      table.reload('idTest', {
        ...
        url:'<%=basePath%>vistor/getvisitorByVisitorId'
         ,where: {
    	...
    	//执行重载
         table.reload('idTest', {
           ...
           url:'<%=basePath%>vistor/getvisitorByVisitorName'
            ,where: {
